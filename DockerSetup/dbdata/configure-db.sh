...
while [[ $DBSTATUS -ne 0 ]] && [[ $i -lt $TRIES ]]; do
	i=$((i+1))
	DBSTATUS=$(/opt/mssql-tools/bin/sqlcmd -S sqlserver -U sa -P Your!Password -Q "SET NOCOUNT ON; Select COALESCE(SUM(state), 0) from sys.databases") || DBSTATUS=1
...
/opt/mssql-tools/bin/sqlcmd -S localhost -U sa -P Your!Password -d master -i docker-entrypoint-initdb.d/InitialCreate.sql
